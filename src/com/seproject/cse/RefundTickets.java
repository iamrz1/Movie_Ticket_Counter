		...
		ActionListener selectAction= new ActionListener(){
			...
			public void actionPerformed(ActionEvent a) {
				...
				if (!targetTicketReads.contains("Booked"))
					JOptionPane.showMessageDialog(null,"Ticket is not Booked");
				else{
					ticketMain[targetIndex]=ticketMain[targetIndex].substring(0,3);
					writeToFile();
					frame.dispose();
					JOptionPane.showMessageDialog(null,"Refund Confirmed");
					readFromTicketFile(targetFile);
					runSelectTicketUI();
...
private void readFromTicketFile(String targetFile){
		...
		try {
			FileReader scFile = new FileReader(targetFile);
			Scanner scan = new Scanner (scFile);
			for (size=0;scan.hasNext();size++){
